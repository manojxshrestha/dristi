...
run a one-shot fingerprint and parse `$DRISTI_ROOT/engagements/recon/<target>/live-hosts.txt` if present:

```bash
curl -sI "https://$TARGET" 2>/dev/null | tr -d '\r'
test -f "$DRISTI_ROOT/engagements/recon/$TARGET/live-hosts.txt" && cat "$DRISTI_ROOT/engagements/recon/$TARGET/live-hosts.txt"
```
...
- **`okta-attacker`** / **`m365-attacker`** / **`vpn-attacker`** / **`vcenter-attacker`** / **`cloud-iam-auditor`** / **`supply-chain-hunter`** / **`apk-analyzer`** — When fingerprint signals match. Workflow primitive: step 1's curl fingerprint scan against `$DRISTI_ROOT/engagements/recon/<target>/live-hosts.txt` maps banner / domain signals to one or more of these platform skills.
