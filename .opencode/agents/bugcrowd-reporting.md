...
5. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="All phases (Bugcrowd Reporting)")`
6. **Track coverage** → `track_test(engagement_id, test_id="All phases (Bugcrowd Reporting)", status="completed", notes=...)`
...
> Companion to the generic `report-writer` skill. Use when working specifically on Bugcrowd submissions where VRT mapping, OOS-clause rebuttals, or per-program target selection matter.

This skill encodes patterns that apply specifically to Bugcrowd's submission flow. For the generic per-platform templates (HackerOne / Bugcrowd / Intigriti / Immunefi report bodies), use the `report-writer` skill. For the 7-Question Gate before deciding to report at all, use `triage-validator`.
...
| "What's the report body template for Bugcrowd?" | `report-writer` |
...
| "Where do I find recon probes for this asset class?" | `osint-gatherer` |

---

## Notes on usage

This skill is small and focused. It does NOT duplicate content from `report-writer` (per-platform templates, CVSS scoring, downgrade counters, 60-second pre-submit checklist) — load both skills together when filing a Bugcrowd report. The two skills' content composes naturally because their boundaries are clean: `report-writer` is the body templates, `bugcrowd-reporter` is the program-specific tactics layered on top.

---

## Related Skills & Chains

- **`report-writer`** — Always load this skill alongside `bugcrowd-reporter`. Workflow primitive: `report-writer` provides the body skeleton; this skill layers VRT selection + severity-request paragraph + OOS-clause rebuttals on top of that skeleton.
