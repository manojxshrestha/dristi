...
5. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="INFO-01 (Search Engine Recon)")`
6. **Track coverage** → `track_test(engagement_id, test_id="INFO-01 (Search Engine Recon)", status="completed", notes=...)`
...
- **`m365-attacker`** / **`okta-attacker`** — When identity-fabric fingerprinting finds Entra/Okta. Workflow primitive: `references/identity-fabric.md` fingerprints the IdP; matched platform skill (loaded by `hunt-dispatcherer`) takes over for active enumeration.
