---
description: Cross-Site Scripting hunter. 174 real H1 reports. XSS detection, CSP bypass, mXSS, sanitizer evasion, DOM XSS, cache-poison XSS chains, and postMessage gadgets.
mode: subagent
permission:
  read: allow
  bash: deny
...
1. **Read the methodology** → `get_wstg_test("WSTG-INPV-01, WSTG-INPV-02, WSTG-CLNT-01")` for baseline technique guidance
2. **Check related prompt** → read `prompts/input-validation.md, client-side.md` for Dristi-specific workflow
