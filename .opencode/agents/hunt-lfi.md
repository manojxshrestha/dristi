...
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="WSTG-INPV-06")`
7. **Track coverage** → `track_test(engagement_id, test_id="WSTG-INPV-06", status="completed", notes=...)`
...
grep -E "(\?|&)(page|file|path|template|view|lang|module|include|doc|load|read|content)=" \
  runtime/engagements/${ENGAGEMENT_ID:-default-engagement}/recon/$TARGET/urls.txt
