...
5. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="CONF-03 (Infrastructure Config)")`
6. **Track coverage** → `track_test(engagement_id, test_id="CONF-03 (Infrastructure Config)", status="completed", notes=...)`
...
| **CVE-2022-42475** | FortiOS 7.x specific | Heap overflow — pre-auth RCE | Complex exploit; manual test required |
| **CVE-2023-27997** (XORtigate) | FortiOS various | Heap overflow — pre-auth RCE | Public PoCs exist |
| **CVE-2024-21762** | FortiOS 6.x-7.x | OOB write — pre-auth RCE | Public PoC available |
...
| **CVE-2022-27518** | ADC/Gateway with SAML configured | Pre-auth RCE | Complex; requires SAML metadata |
...
| **CVE-2021-22893** | Pulse Connect Secure 9.x | Pre-auth RCE | Complex multi-step; manual test required |
...
| CVE | Affects | Type | Test |
|---|---|---|---|
| **CVE-2021-20016** | SMA 100 series specific firmware | SQL injection — pre-auth | Manual SQLi probe |
...
Manually probe known CVE endpoints using curl. Focus on behavioral fingerprints (response differences between patched/unpatched) rather than version banners.
...
- **Don't skip manual probing for known CVEs** — test 3+ CVEs per vendor before concluding patched
