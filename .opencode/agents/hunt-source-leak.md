...
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="INFO-06 (Information Leak)")`
7. **Track coverage** → `track_test(engagement_id, test_id="INFO-06 (Information Leak)", status="completed", notes=...)`
...
This agent has a corresponding reference library at `knowledge/payloads/Insecure Source Code Management/` (50 lines).
