...
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="WSTG-APIT-02")`
7. **Track coverage** → `track_test(engagement_id, test_id="WSTG-APIT-02", status="completed", notes=...)`
...
### Default discovery paths (cross-references `web-recon`)
...
2. httpx-probe every path above across the full subdomain set; flag 200 with `Content-Type: application/json` AND body matching `"swagger"` or `"openapi"`.
3. For every hit: `jq '.paths | keys' swagger.json` → feed to kiterunner / Autorize.
4. `jq '.components.schemas' swagger.json` → mass-assignment field candidates.
5. Banner the Swagger UI HTML for version string; map to the CVE-2018-25031 / DOM-XSS table.
6. Test `?configUrl=` and `?url=` parameter handling on every Swagger UI hit.
