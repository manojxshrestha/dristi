...
2. **Check related prompt** → read `templates/testing-strategies.md` for Dristi-specific workflow
...
5. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="All phases (Evidence Handling)")`
6. **Track coverage** → `track_test(engagement_id, test_id="All phases (Evidence Handling)", status="completed", notes=...)`
...
- **`bb-methodology`** — When Phase 11–12 (Validate & Report) needs evidence captured. Workflow primitive: the evidence-capture step routes through this skill before the report is drafted.
