...
3. **BurpSuite pro workflow — See [Burp Suite Flow](../docs/burp-flow.md) for full Burp MCP tool reference (proxy, repeater, intruder, collaborator, scanner, organizer) and per-phase workflow. **DOM technique**: Use `burp_create_repeater_tab()` combined with a real browser to test postMessage listeners, `location.hash`/`location.search` sinks, and DOM clobbering. Install DOM-Invader extension for automated sink identification. Confirm execution in Chrome/Firefox — Burp alone cannot verify DOM XSS.
4. **Playwright browser — SPA DOM analysis**: Use `playwright_browser_navigate` to the SPA (Angular/EmberJS/React), then `playwright_browser_evaluate` to enumerate client-side routes, inspect `window.__INITIAL_STATE__`, and test DOM clobbering sinks. SPAs only reveal their full attack surface in a real browser JS context. See [Browser Testing](../docs/browser-testing.md).

5. **Find vulnerabilities** → `log_finding()` or `findings_add_vuln()` to persist to SQLite
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="WSTG-CLNT-01")`
7. **Track coverage** → `track_test(engagement_id, test_id="WSTG-CLNT-01", status="completed", notes=...)`
...
This agent has a corresponding reference library at `knowledge/payloads/DOM Clobbering/` (146 lines).
...
- **Bypass Patterns**: WAF/filter evasion specific to DOM
...
grep -r "addEventListener.*message\|postMessage" runtime/engagements/${ENGAGEMENT_ID:-default-engagement}/recon/$TARGET/ --include="*.js" 2>/dev/null | \
...
grep -r "dangerouslySetInnerHTML" runtime/engagements/${ENGAGEMENT_ID:-default-engagement}/recon/$TARGET/ --include="*.js" 2>/dev/null
...
grep -r "angular\|vue\|handlebars\|mustache\|nunjucks" runtime/engagements/${ENGAGEMENT_ID:-default-engagement}/recon/$TARGET/ --include="*.js" 2>/dev/null | head -5
