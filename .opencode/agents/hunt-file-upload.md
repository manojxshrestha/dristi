...
4. **Playwright browser — SVG/HTML execution verification**: After uploading a crafted SVG or HTML file, use `playwright_browser_navigate` to the uploaded file URL. Use `playwright_browser_console_messages` to catch XSS alert() or JS execution. Use `playwright_browser_take_screenshot` for visual PoC. File-upload XSS only fires in a browser — curl cannot render SVG/HTML. See [Browser Testing](../docs/browser-testing.md).

5. **Find vulnerabilities** → `log_finding()` or `findings_add_vuln()` to persist to SQLite
6. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="WSTG-BUSL-07")`
7. **Track coverage** → `track_test(engagement_id, test_id="WSTG-BUSL-07", status="completed", notes=...)`
