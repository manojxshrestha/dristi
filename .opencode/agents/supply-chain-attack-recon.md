...
5. **Log findings** → `findings_add_vuln(engagement_id, title, severity, ..., test_id="CONF-12 (Supply Chain Security)")`
6. **Track coverage** → `track_test(engagement_id, test_id="CONF-12 (Supply Chain Security)", status="completed", notes=...)`
...
# Compare versions to osv.dev for known vulns
...
- **`osint-gatherer`** — Recon discipline overlaps heavily; SBOMs, JS bundles, GitHub org enumeration, Docker registry tags all live in both. Chain primitive: `osint-gatherer` GitHub-org recon yields internal package names referenced in CI workflows → `supply-chain-hunter` cross-references these against public npm/PyPI for typosquat/confusion candidates.
