...
curl -s "https://crt.sh/?q=%.${TARGET}&output=json" \
  | jq -r '.[].name_value' \
  | sed 's/\*\.//g' \
  | sort -u > recon/$TARGET/subdomains.txt 2>/dev/null || true
...
grep -v -E '^https://[a-z]{2}\.' recon/$TARGET/live-hosts.txt \
  > recon/$TARGET/live-hosts-filtered.txt 2>/dev/null || true
```
