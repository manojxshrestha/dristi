...
if [ -x "$HTTPX_BIN" ] && [ -s "$RECON_DIR/subdomains/all.txt" ]; then
    ...
    # Phase 2b: Filter out language/locale subdomains (2-letter ISO codes like fr, ja, zh, hi, ne, de, es)
    # These are the same app with translated content — no unique attack surface.
    if [ -s "$RECON_DIR/live/urls.txt" ]; then
        log_step "Filtering out language/locale subdomains..."
        local LANG_PATTERN='^https://[a-z]{2}\.(.*\.)?'"$(printf '%s' "$TARGET" | sed 's/\./\\./g')"
        grep -v -E "$LANG_PATTERN" "$RECON_DIR/live/urls.txt" > "$RECON_DIR/live/urls_deduped.txt" 2>/dev/null || true
        grep -v -E "$LANG_PATTERN" "$RECON_DIR/live/httpx_full.txt" > "$RECON_DIR/live/httpx_full_deduped.txt" 2>/dev/null || true

        # Also filter out xxx-xxx.airbnb.com patterns (locale test pages)
        # Keep only the first occurrence of each hostname
        awk '!seen[host]++ {host=$1; print}' "$RECON_DIR/live/urls_deduped.txt" > "$RECON_DIR/live/urls_unique.txt" 2>/dev/null || true

        DEDUPED_COUNT=$(wc -l < "$RECON_DIR/live/urls_unique.txt" 2>/dev/null || echo 0)
        log_done "Unique non-locale hosts: $DEDUPED_COUNT"
    fi

    # Phase 2c: Cariddi — secrets, info disclosure, endpoints, errors, juicy extensions
    if command -v cariddi &>/dev/null; then
        log_step "Running cariddi for secrets, info disclosure, endpoints, errors..."
        mkdir -p "$RECON_DIR/cariddi"
        cariddi -intensive -s -info -e -err -ext 1 \
            -c 30 -d 1 -plain \
            -oh "$RECON_DIR/cariddi/cariddi.html" \
            -ot "$RECON_DIR/cariddi/cariddi.txt" \
            < "$RECON_DIR/live/urls.txt" 2>/dev/null || true
        CARIDDI_FINDINGS=$(wc -l < "$RECON_DIR/cariddi/cariddi.txt" 2>/dev/null || echo 0)
        log_done "Cariddi findings: $CARIDDI_FINDINGS"

        # Phase 2d: Parse Cariddi output into structured candidate files
        if [ -s "$RECON_DIR/cariddi/cariddi.txt" ]; then
            log_step "Parsing Cariddi findings into candidate classes..."

            # Secrets / API keys / tokens
            grep -iE "(key|secret|token|password|api_key|bearer|auth|jwt|sk-[a-zA-Z0-9]|AKIA)" \
                "$RECON_DIR/cariddi/cariddi.txt" \
                > "$RECON_DIR/cariddi/secrets.txt" 2>/dev/null || true

            # Info disclosures (emails, internal IPs, stack traces, debug paths)
            grep -iE "(email|@|stack trace|debug|internal IP|10\.|192\.168|172\.|localhost|127\.0)" \
                "$RECON_DIR/cariddi/cariddi.txt" \
                > "$RECON_DIR/cariddi/info_disclosure.txt" 2>/dev/null || true

            # Error pages (stack traces, error codes, exception messages)
            grep -iE "(error|exception|warning|fatal|stack trace|line |file not found|undefined|500|404)" \
                "$RECON_DIR/cariddi/cariddi.txt" \
                > "$RECON_DIR/cariddi/errors.txt" 2>/dev/null || true

            # Endpoints / paths from cariddi
            grep -E "(/api/|/v1/|/v2/|/v3/|/graphql|/admin|/internal|/debug|/test|/staging|/dev)" \
                "$RECON_DIR/cariddi/cariddi.txt" \
                > "$RECON_DIR/cariddi/endpoints.txt" 2>/dev/null || true

            # Merge cariddi endpoints into the main api-endpoints candidate file
            if [ -s "$RECON_DIR/cariddi/endpoints.txt" ]; then
                cat "$RECON_DIR/cariddi/endpoints.txt" \
                    >> "$RECON_DIR/urls/api_endpoints.txt" 2>/dev/null || true
            fi

            CARIDDI_SECRETS=$(wc -l < "$RECON_DIR/cariddi/secrets.txt" 2>/dev/null || echo 0)
            CARIDDI_DISCLOSURE=$(wc -l < "$RECON_DIR/cariddi/info_disclosure.txt" 2>/dev/null || echo 0)
            CARIDDI_ERRORS=$(wc -l < "$RECON_DIR/cariddi/errors.txt" 2>/dev/null || echo 0)
            CARIDDI_ENDPOINTS=$(wc -l < "$RECON_DIR/cariddi/endpoints.txt" 2>/dev/null || echo 0)
            log_done "  Secrets: $CARIDDI_SECRETS | Info disclosure: $CARIDDI_DISCLOSURE | Errors: $CARIDDI_ERRORS | Endpoints: $CARIDDI_ENDPOINTS"
        fi
    fi
else
