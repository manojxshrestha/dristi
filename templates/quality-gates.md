...
6. **Tool-only testing**: Running sqlmap but not doing manual SQLi testing. Tools miss context-specific injection points. Always do manual testing first.
...
- [ ] CLI tools launched against ALL domains with server-side endpoints (sqlmap, dalfox, commix, sstimap, crlfuzz, smuggler)
