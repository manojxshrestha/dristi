...
All **92 skills** in the bundle, grouped by domain. Each auto-loads when your prompt matches its trigger keywords — no need to invoke by name. Use your browser's find (⌘/Ctrl-F) or the docs search box.

> Generated by `scripts/gen_skill_catalog.py` — do not edit by hand.

## Hunt — web app vuln classes (52)
...
| `waf-bypass-aws` | Skill for bypassing AWS WAF using encoding techniques, managed rule weaknesses, and known bypass payloads. | 10 |
| `waf-bypass-cloudflare` | Skill for bypassing Cloudflare WAF using specific XSS payloads, event handler techniques, and known bypass patterns. | 15 |
| `waf-bypass-f5` | Skill for bypassing F5 BIG-IP ASM WAF using XSS encoding, XXE exploitation, directory traversal, and known bypass patterns. | 10 |
| `waf-bypass-imperva` | Skill for bypassing Imperva Incapsula WAF using XSS encoding, SQLi parameter pollution, and known bypass patterns from the Awesome-WAF knowledge base. | 15 |
| `waf-bypass-modsecurity` | Skill for bypassing ModSecurity WAF, especially OWASP CRS rulesets, using encoding obfuscation, comment injection, and paranoia-level specific techniques. | 15 |
| `waf-bypass-sucuri` | Skill for bypassing Sucuri CloudProxy WAF using event handler XSS, HTTP smuggling, and known bypass patterns. | 8 |
| `waf-encoding-obfuscation` | Skill for obfuscating payloads using various encoding techniques to bypass WAF signature detection. | 25 |
| `waf-evasion-rce` | Skill for bypassing WAF command injection and RCE filters using encoding, wildcards, alternative shells, and request smuggling. | 20 |
| `waf-evasion-sqli` | Skill for bypassing WAF SQL injection filters using encoding, comment injection, alternative operators, and parameter manipulation. | 40 |
| `waf-evasion-xss` | Skill for bypassing WAF XSS filters using encoding techniques, event handler abuse, structural manipulation, and browser-specific quirks. | 50 |
| `waf-fingerprinting` | Skill for identifying Web Application Firewall vendors through HTTP response analysis, cookie inspection, header examination, and side-channel techniques. | 110 |
| `waf-header-spoofing` | Skill for bypassing WAF restrictions by spoofing HTTP headers to impersonate internal IPs, trusted clients, or specific user-agents. | 10 |
| `waf-hpp-hpf` | Skill for bypassing WAFs using HTTP Parameter Pollution (HPP) and HTTP Parameter Fragmentation (HPF) techniques. | 15 |
| `waf-protocol-evasion` | Skill for bypassing WAFs using HTTP protocol-level manipulation including SSL/TLS cipher selection, HTTP/0.9 downgrade, request smuggling, and chunked encoding abuse. | 10 |
| `waf-regex-reversing` | Skill for reverse-engineering WAF regex rules by progressively testing blocked keywords and finding equivalent alternatives. | 10 |
